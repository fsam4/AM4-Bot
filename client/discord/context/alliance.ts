    ...
    async execute(interaction, { database, rest, locale }) {
        ...
        try {
            ...
            if (allianceDocument && isCachedUserContextMenuInteraction(interaction)) {
                ...
                const recentlyLeft = memberDocuments.filter(member => member.left).sort((a, b) => compareAsc(b.left, a.left)).slice(0, 5);
                if (recentlyLeft.length) {
                    embed.addFields({
                        name: Formatters.bold(Formatters.underscore("Recently left")),
                        value: Formatters.codeBlock(recentlyLeft.map(member => `${member.name} (${formatDistanceToNowStrict(member.left, { addSuffix: true, unit: 'day' })})`).join('\n')),
                        inline: true
                    });
                }
