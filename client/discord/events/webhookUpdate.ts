...
const event: Event = {
    ...
    async execute(channel, { database, client, webhook, log }) {
        ...
        if (permissions.has("MANAGE_WEBHOOKS")) {
            ...
            const webhooks = await channel.fetchWebhooks().then(webhooks => {
                const systemWebhooks = [webhook.id, log.id];
                return webhooks.filter(webhook => webhook.owner.id === client.user.id && !systemWebhooks.includes(webhook.id));
            });
            if (webhooks.size) {
